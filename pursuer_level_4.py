...
# Evader can only vertically or horizontally 
# speed: 0.75*pursuer speed

class PursuerLevelTwo(gym.Env):
    metadata = {
    ...
    def step(self, action):
    	...
    	if self.region == 0:
    		if self.flag == 1:
    			de_x = 0.0
    			de_y = 0.6*self.tau
    			if self.e_y + de_y >= 98.0:
    				de_y = -0.6*self.tau
    				self.flag = -1
    		elif self.flag == -1:
    			de_x = 0.0
    			de_y = -0.6*self.tau
    			if self.e_y + de_y <= 2.0:
    				de_y = 0.6*self.tau
    				self.flag = 1
    		else:
    		...
    		if self.flag == 1:
    			de_y = 0.0
    			de_x = 0.6*self.tau
    			if self.e_x + de_x >= 98.0:
    				de_x = -0.6*self.tau
    				self.flag = -1
    		elif self.flag == -1:
    			de_y = 0.0
    			de_x = -0.6*self.tau
    			if self.e_x + de_x <= 2.0:
    				de_x = 0.6*self.tau
    				self.flag = 1
    		else:
    			v = random.randint(0,1)
    ...
    def reset(self):
    	...
    	self.region = random.randint(0,1)
    	if self.region == 0:
    		region_x = random.randint(0, 5)
    		if region_x == 0:
    			self.e_x = random.uniform(0.0, 5.0-1e-7)
    			self.e_y = random.uniform(0.0, 100.0)
    		elif region_x >= 1 and region_x <= 4:
    			self.e_x = random.uniform(45+1e-7, 65.0-1e-7)
    			self.e_y = random.uniform(0.0, 100.0)
    		elif region_x == 5:
    			self.e_x = random.uniform(95.0+1e-7, 100.0)
    			self.e_y = random.uniform(0.0, 100.0)
    	else:
    		region_y = random.randint(0, 5)
    		if region_y == 0:
    			self.e_x = random.uniform(0.0, 100.0)
    			self.e_y = random.uniform(0.0, 5.0-1e-7)
    		elif region_y >= 1 and region_y <= 4:
    			self.e_x = random.uniform(0.0, 100.0)
    			self.e_y = random.uniform(45+1e-7, 65.0-1e-7)
    		elif region_y == 5:
    			self.e_x = random.uniform(0.0, 100.0)
    			self.e_y = random.uniform(95.0+1e-7, 100.0)
